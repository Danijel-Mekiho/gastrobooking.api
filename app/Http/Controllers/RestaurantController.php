...
class RestaurantController extends Controller
{
    ...
    public function getFirst(Request $request){
        // $restaurant = $this->restaurantRepository->all($request, $this->itemPerPage)->searched_data;
        // $recommended_restaurant = $this->restaurantRepository->all($request, $this->itemPerPage)->bestmatch;
        
        // $response1 = $this->response->paginator($restaurant, new RestaurantTransformerWithLogo($this->restaurantRepository));
        // $response2 = $this->response->collection($recommended_restaurant, new RestaurantTransformerWithLogo($this->restaurantRepository));

        $restaurant = $this->restaurantRepository->getRecommended($request, $this->itemPerPage);
    ...
    public function detail($restaurant_id, $type){
        $this->restaurantRepository->detailClicked($restaurant_id, $type);
        $restaurant = $this->restaurantRepository->find($restaurant_id);
        if(empty($restaurant)){
...
//    }
    ...
    public function getMenuList(Request $request){
        return $this->restaurantRepository->getMenuList($request);
    }

    public function getSetting(Request $request){
        return $this->restaurantRepository->getSetting($request);
    }

    public function restaurantRequest($restaurant_id){
       $status = 'N';
       $response = $this->restaurantRepository->restaurantRequest($restaurant_id, $status);
       return $response;
    }
    public function restaurantPendingRequest($restaurant_id){
       $status = 'P';
       $response = $this->restaurantRepository->restaurantRequest($restaurant_id, $status);
       return $response;
    }
    public function restaurantConfirmRequest($restaurant_id){
       $status = 'C';
       $response = $this->restaurantRepository->restaurantRequest($restaurant_id, $status);
       return $response;
    }
    public function restaurantCancelRequest($restaurant_id){
       $status = 'CA';
       $response = $this->restaurantRepository->restaurantRequest($restaurant_id, $status);
       return $response;
    }

    public function cancelRestaurantRequest(Request $request){
       $response = $this->restaurantRepository->cancelRestaurantRequest($request);
       return $response;
    }

    public function updateRequest(Request $request){
       return $this->restaurantRepository->updateRequest($request);
    }

    public function changeStatus($request_id){
        return $this->restaurantRepository->changeStatus($request_id);
    }

    public function restaurantCountUnreadRequest($restaurant_id){
        return $this->restaurantRepository->restaurantCountUnreadRequest($restaurant_id);
    }

    public function currentRequests($restaurant_id, $serve_at){
        return $this->restaurantRepository->currentRequests($restaurant_id, $serve_at);
    }

    public function countMissingBooking($restaurantId){
        return $this->restaurantRepository->countMissingBooking($restaurantId);
    }
}
